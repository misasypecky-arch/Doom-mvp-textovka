...
public class CommandHandler
{
    ...
    private string CmdMluv(Hrac hrac, string[] args)
    {
        if (args.Length == 0) return "S kým chceš mluvit? (např. 'mluv obchodnik_dusi')";
        ...
        return $"--- Začal jsi mluvit s: {npc.Name} ---\n" +
               "1 - Kdo jsi?\n" +
               "2 - Co tady děláš?\n" +
               "3 - (Odejít z konverzace)\n" +
               "Vyber číslo odpovědi:";
    }

    private string ZpracujDialog(Hrac hrac, string vstup)
    {
        string volba = vstup.Trim();
        string npcId = hrac.DialogNpcId;

        if (volba == "3" || volba.ToLower() == "odejit" || volba.ToLower() == "konec")
        {
            hrac.IsInDialog = false;
            hrac.DialogNpcId = null;
            return "Ukončil jsi rozhovor.";
        }

        Npc npc = NajdiNpcPodleId(npcId);
    ...
    private string PouzijItem(Hrac hrac, string itemNameRaw)
    {
     ...
     // Najdeme předmět u hráče v inventáři
     Item predmet = hrac.Inventory.Items.FirstOrDefault(i => i.ItemId == itemName || i.Name.ToLower() == itemName);

     if (predmet == null)
        return "Tento předmět u sebe nemáš.";

     // Vykonáme efekt
     string vysledek = predmet.VykonejEfekt(hrac);

     // Pokud předmět něco udělal (není to jen "nic"), smažeme ho z inventáře (spotřební věc)
     // Pokud chceš, aby třeba štít nezmizel, musel bys do Itemu přidat bool IsConsumable.
     if (predmet.EffectId != "nic")
     {
        hrac.Inventory.RemoveItem(predmet.ItemId);
     }
